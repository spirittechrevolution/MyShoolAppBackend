  ...
  constructor() {
    ...
    if (!this.isMockMode() && process.env.NODE_ENV !== 'test') {
      if (!this.apiKey || !this.merchantKey || !this.merchantId) {
        console.error('❌ Orange Money: Credentials manquants!');
        console.error('   Veuillez configurer: ORANGE_MONEY_API_KEY, ORANGE_MONEY_MERCHANT_KEY, ORANGE_MONEY_MERCHANT_ID');
